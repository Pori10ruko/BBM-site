...
const WorkCard: React.FC<WorkCardProps> = ({ work, onClick }) => {
  ...
  return (
    <motion.div
      ...
      className="group relative cursor-pointer flex flex-col space-y-5"
    >
      <div className="relative aspect-[4/3] overflow-hidden bg-gray-100">
        {work.imageUrl ? (
          <motion.img
            ...
            alt={title}
            loading="lazy"
            className="w-full h-full object-cover grayscale-0 [@media(hover:hover)and(pointer:fine)]:grayscale [@media(hover:hover)and(pointer:fine)]:group-hover:grayscale-0 transition-all duration-700 ease-in-out scale-100 group-hover:scale-105"
          />
        ) : (
      ...
      <div className="space-y-3">
        ...
        <h3 className="text-xl font-serif font-bold text-black group-hover:text-[#C9A66B] transition-colors leading-tight">{title}</h3>
      ...
      <div className="pt-1 flex gap-4 items-center">
        <span className="text-[10px] font-bold tracking-[0.3em] uppercase text-gray-300 group-hover:text-black transition-all">Details</span>
        {primaryLink && (
          <a
            href={primaryLink.url}
            target="_blank"
            rel="noopener noreferrer"
            className="text-[10px] font-bold tracking-[0.2em] uppercase text-[#C9A66B] hover:text-black transition-colors border-b border-[#C9A66B]/30 hover:border-black pb-0.5"
